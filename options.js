...
function save_data(){
    ...
    $('div.path').each(function(){
        var inputs = $(this).find('input'),
            tmp = {title: inputs.eq(0).val(), path: inputs.eq(1).val(), list_numbers: inputs.eq(2).val()};
        console.log(tmp);
        if (tmp.path!='' && tmp.list_numbers!=''){
            var splitted_list_numbers = tmp.list_numbers.split(',');
            tmp.list_numbers = $.grep(splitted_list_numbers, function(v, i){
                return v != '';
            })
